...
def plot_csv_amperage(a):
	...
	ax1.grid(True)
	ax1.plot(np.array(data[keys[3]]), np.array(data[keys[1]]), 'ro', np.array(data[keys[3]]), func_1, 'k', 
			 np.array(data[keys[3]]), np.array(data[keys[2]]), 'bo', np.array(data[keys[3]]), func_3, 'g')
...
def plot_csv_amperage_all(a):
	...
	for data in a:
		...
		func_1 = parametr_1[0]*np.array(data[keys[3]])+parametr_1[1]
		func_3 = parametr_3[0]*np.array(data[keys[3]])+parametr_3[1]
		ax1.plot(np.array(data[keys[3]]), np.array(data[keys[1]]), 'ro', label = keys[1])
		ax1.plot(np.array(data[keys[3]]), func_1, 'k', label = 'func_for_' + keys[1])
		ax1.plot(np.array(data[keys[3]]), np.array(data[keys[2]]), 'bo', label = keys[2])
		ax1.plot(np.array(data[keys[3]]), func_3, 'g', label = 'func_for_' + keys[2])
	ax1.legend()
...
def plot_csv_half_sum(a):
	...
	ax1 = fig.add_subplot(111)
	ax1.grid(True)
	ax1.plot(np.array(data[keys[3]]), (np.array(data[keys[1]]) + np.array(data[keys[2]]))/2, 'ro') 
	ax1.plot(np.array(data[keys[3]]), func, 'k') 
	ax1.set_xlabel(keys[3] + ', %')
...
def plot_csv_half_sum_all(a):
	...
	for data in a:
		...
		parametr_1, parametr_cov_1 = curve_fit(approk, np.array(data[keys[3]]), (np.array(data[keys[1]]) + np.array(data[keys[2]]))/2)
		func = parametr_1[0]*np.array(data[keys[3]])+parametr_1[1]
		ax1.plot(np.array(data[keys[3]]), (np.array(data[keys[1]]) + np.array(data[keys[2]]))/2, 'ro', label = f'({keys[1]} + {keys[2]})/2') 
		ax1.plot(np.array(data[keys[3]]), func, 'k', label = 'func_for_' + f'({keys[1]} + {keys[2]})/2') 
	ax1.set_xlabel(u'offset, %')
	ax1.set_ylabel(u'$(I_{1} + I_{2})/2$')
	ax1.legend()
	ax1.set_title('all') 
